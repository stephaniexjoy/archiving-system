  ...
  if (status === "authenticated") {
    ...
    return (
      <aside className={`flex place-content-center bg-[#5B0505] text-white ${isMinimized ? "w-[10vh]" : "w-[24vh]"} h-full transition-all`}>
        <nav>
          <div>
            <div className={`mb-2 h-10 px-2 cursor-pointer ${isMinimized ? "rotate-180" : ""}`} onClick={handleToggleMinimize}>
              <IoMdMenu className="text-white text-2xl mt-2 " />
            </div>
            <div className={`h-10 text-center mt-4 py-2 text-base center ${isMinimized ? "hidden" : ""}`}>
              {userImage}
          ...
          <ul className="mt-24">
            {session.user.position === "Faculty" && (
              <>
                <MenuItem path="/dashboard" icon={<FaHome className="text-white text-2xl center" />} label="HOME" isMinimized={isMinimized} />
                <MenuItem path="/dashboard/profile" icon={<IoMdPerson className="text-white text-2xl center " />} label="PROFILE" isMinimized={isMinimized} />
                <MenuItem path="/dashboard/archiving" icon={<FaBook className="text-white text-2xl center" />} label="ARCHIVING" isMinimized={isMinimized} />
