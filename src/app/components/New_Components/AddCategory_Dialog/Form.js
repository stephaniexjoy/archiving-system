        ...
        action={async (formData) => {
          ...
          if (isAdded_Material) {
            if (isAdded_Material === "existingData") {
              ...
              toast({
                description:
                  "You have successfully added a new Filter (Material/Course).",
      ...
      >
        <div className="flex flex-col justify-center w-full max-w-md items-center space-x-2 ">
          <Input
            ...
            className="font-semibold text-black border border-black mt-4"
          />
          <Button type="submit" className="mt-8 bg-[#8F8F8F] px-5 font-bold">
            ADD CATEGORY{" "}
          </Button>
        </div>
      </form>
